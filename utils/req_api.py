...
class RequestsAPI:
    ...
    def _currency_prices_usd_url(self, ids):
        return ROOT + f"/simple/price?ids={ids}&vs_currencies=usd"

    async def get_currency_prices_usd(self, ids, msg_working: Message = None):
        return await self._get_request(self._currency_prices_usd_url(ids), msg_working=msg_working)
